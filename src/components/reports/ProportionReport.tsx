...
export const ProportionReport: React.FC<ProportionReportProps & { companyName?: string; companyLogo?: string; config?: any }> = ({ formula, quantidade, unidade, embalagemVolume = 1, insumosData = [], companyName, companyLogo, config }) => {
  ...
  const nonChemicals = (formula.insumos || []).filter(i => !i.quimico);
      ...
      <div className="space-y-8">
        <div className="grid grid-cols-2 gap-8">
          ...
          <div className="bg-blue-600 text-white p-4 rounded-xl shadow-md text-center">
            ...
            <p className="text-xs opacity-70 mt-1">Volume total: {totalVolume.toLocaleString('pt-BR')}L</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
            ...
            <p className="text-lg font-bold text-slate-700">R$ {(quantidade > 0 ? custoTotal / quantidade : 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
          </div>
            ...
            <table className="w-full text-left border-collapse border border-slate-200 rounded-lg overflow-hidden">
                ...
                {chemicals.map(insumo => {
                  ...
                  return (
                    <tr key={insumo.id} className="text-xs">
                      ...
                      <td className="px-4 py-3 text-center font-black text-amber-700 bg-amber-50/30 font-mono">{qtdAjustada.toLocaleString('pt-BR', { minimumFractionDigits: 3 })} {insumo.unidade}</td>
              ...
              <tfoot>
                <tr className="bg-amber-50 font-bold border-t-2 border-slate-200">
                  ...
                  <td className="px-4 py-3 text-right text-amber-700">R$ {chemicals.reduce((s, i) => s + getQuantidadeAjustada(i) * i.valorUnitario, 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                </tr>
              </tfoot>
            </table>
          </div>
            ...
            <table className="w-full text-left border-collapse border border-slate-200 rounded-lg overflow-hidden">
                ...
                {nonChemicals.map(insumo => {
                  ...
                  return (
                    <tr key={insumo.id} className="text-xs">
                      ...
                      <td className="px-4 py-3 text-center text-slate-500 font-mono">R$ {price.toFixed(2)}</td>
                      ...
                      <td className="px-4 py-3 text-right font-bold text-slate-700">R$ {(qtdAjustada * price).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="bg-blue-50 font-bold border-t-2 border-slate-200">
                  ...
                  <td className="px-4 py-3 text-right text-blue-700">R$ {nonChemicals.reduce((s, i) => s + getQuantidadeAjustada(i) * getEffectivePrice(i).price, 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</td>
                </tr>
              </tfoot>
            </table>
          </div>
        ...
        <div className="bg-slate-800 text-white p-5 rounded-xl">
          <div className="grid grid-cols-3 gap-4">
            <div className="text-center">
              ...
              <p className="text-xl font-black">R$ {custoTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
            </div>
            <div className="text-center">
              <p className="text-[9px] uppercase tracking-widest opacity-60 mb-1">Custo por Unidade ({embalagemVolume}L)</p>
              <p className="text-xl font-black">R$ {(quantidade > 0 ? custoTotal / quantidade : 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
            </div>
            <div className="text-center">
              <p className="text-[9px] uppercase tracking-widest opacity-60 mb-1">Custo por Litro</p>
              <p className="text-xl font-black">R$ {(totalVolume > 0 ? custoTotal / totalVolume : 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</p>
            </div>
          </div>
        </div>
